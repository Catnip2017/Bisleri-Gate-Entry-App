...
PROMPTS = {
    ...
    "blow_molder": """You are an industrial OCR expert analyzing a KOSME Blow Molder HMI screen.
...
{
  "asset_number": "physical label on machine housing (e.g. FA-PM-452) — read all alphanumeric characters exactly as printed",
  "bottle_recipe": "full text string from the orange top bar (e.g. 0.25l | 16000 BpH | 8g | chemco new)",
  "total_preforms": "Preforms Total counter — plain integer, no commas, no units (e.g. 685376620)",
  "shift_preforms": "Preforms Shift counter — plain integer (e.g. 46454)",
  "total_bottles": "Bottles Total counter — plain integer (e.g. 682074850)",
  "shift_bottles": "Bottles Shift counter — plain integer (e.g. 46312)",
  "operating_hours": "Operating hours value — plain integer, ignore the h unit suffix (e.g. 50694)"
}""",

    "bottling": """You are an industrial OCR expert analyzing a KOSME bottling or filling machine HMI screen.
...
{
  "asset_number": "physical label on machine housing (e.g. FA-BT-123) — read all alphanumeric characters exactly as printed, null if not visible",
  "total_bottles": "lifetime total bottles filled — the largest counter integer on screen, plain integer no commas no units",
  "production_speed_bph": "current production speed in bottles per hour — plain integer only (e.g. 15000)"
}""",
...
{
  "asset_number": "physical label or ID on machine housing if visible (e.g. FA-LB-101) — null if not visible",
  "labels_count": "the main counter number shown on the screen — plain integer no commas no units (e.g. 155512)",
  "format": "format or recipe text visible on screen (e.g. F2 250 ML BISLERI WATER) — null if not visible"
}""",

    "case_counter": """You are an industrial OCR expert analyzing a Smitec shrink wrap or packing machine HMI screen.
...

{
  ...
  "packs_counter": "Packs Counter value — plain integer only (e.g. 1011)",
  "format": "format or recipe text shown on screen (e.g. 8x6 250 ml film only)"
}""",
}
...
def extract_machine_data(
    ...
    if capture_type == "blow_molder":
        return {
            ...
            "total_preforms":  _safe_int(parsed.get("total_preforms")),
            "shift_preforms":  _safe_int(parsed.get("shift_preforms")),
            "total_bottles":   _safe_int(parsed.get("total_bottles")),
            "shift_bottles":   _safe_int(parsed.get("shift_bottles")),
            "operating_hours": _safe_int(parsed.get("operating_hours")),
        }
...
def extract_machine_data_from_image(image_bytes: bytes,
    ...
    return {
        "preform_total":   result.get("total_preforms"),
        "preform_shift":   result.get("shift_preforms"),
        "bottles_total":   result.get("total_bottles"),
        "bottles_shift":   result.get("shift_bottles"),
