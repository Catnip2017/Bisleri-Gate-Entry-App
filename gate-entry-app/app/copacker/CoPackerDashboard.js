...
const CoPackerDashboard = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const adminAccess = roles.includes('itadmin') || roles.includes('copacker');
      if (!adminAccess) {
  ...
  useEffect(() => {
    // IT Admin can browse any location; copacker uses their assigned location
    const loc = userData?.copackerLocation;
    if (loc) {
      loadSessions(loc, selectedDate);
    } else if (isITAdmin) {
      // IT Admin without a fixed location — sessions load when they pick a location
    }
  }, [userData]);
        ...
        <View style={{ padding: 10 }}>
          ...
          {capture.capture_type === 'blow_molder' && (
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 4 }}>
              ...
              <FieldChip label="Last Hour Production"  value={fmtNum(capture.last_hour_production_count)} />
              <FieldChip label="Current Batch"        value={fmtNum(capture.production_count_current_batch)} />
              <FieldChip label="Total Production"     value={fmtNum(capture.total_production_count)} />
              <FieldChip label="Good Bottles"         value={fmtNum(capture.good_bottles_count)} />
              <FieldChip label="Preforms Processed"   value={fmtNum(capture.preforms_processed_count)} />
              <FieldChip label="Target Quantity"      value={fmtNum(capture.target_batch_quantity)} />
            </View>
          )}
  ...
  const renderOcrResult = (result) => {
    ...
    if (result.capture_type === 'blow_molder') {
      ...
      lines.push(['Last Hour Production',  fmtNum(result.last_hour_production_count)]);
      lines.push(['Current Batch',        fmtNum(result.production_count_current_batch)]);
      lines.push(['Total Production',     fmtNum(result.total_production_count)]);
      lines.push(['Good Bottles',         fmtNum(result.good_bottles_count)]);
      lines.push(['Preforms Processed',   fmtNum(result.preforms_processed_count)]);
      lines.push(['Target Quantity',      fmtNum(result.target_batch_quantity)]);
